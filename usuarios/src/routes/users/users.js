...
router.post("/login", (req, res) => {
    ...
    data.query("select * from usuario where nomeusuario=?", req.body.nomeusuario, (error, result) => {
        if (error || result[0] == null) {
            ...
            return res.status(400).send({ msg: "Usuario ou senha incorreto" })
        }

        bcrypt.compare(sh, result[0].senha, (err, same) => {
            if (err) {
                return res.status(500).send({ msg: "Erro ao processar login" })
            }
            else if (same == false) {
                return res.status(400).send({ msg: "usuario ou senha incoreta" })
            }
            else {
